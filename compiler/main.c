...
void const_func(char(*tokens)[10], int *memory, int *memory_index){
	...
	if (symbol_index == 0){
		symbol_tab[symbol_index]->address = 8;
		strcpy(symbol_tab[symbol_index]->variable_name, tokens[1]);
		symbol_tab[symbol_index]->size = 0;
	}
	else{
		strcpy(symbol_tab[symbol_index]->variable_name, tokens[1]);
		symbol_tab[symbol_index]->size = 0;
...
void mov_func(char *param,int instruction_no){
	...
	if (dest[1] == 'X' && dest[0] >= 'A' && dest[0] <= 'H'){ // destination is register
		intermediate_table[intermediate_index]->opcode = 1;
...
void endif_func(int instruction_no, int *stack, int *top){
	...
	i++;
	intermediate_table[i]->parameters[0] = instruction_no + 1;
...
int main(){
	...
	int memory_index = 7;  // 0 to 7 are already reserved
	...
	ending: display_intermediate_table();
	display_block_table();
	getchar();
	return 0;
}